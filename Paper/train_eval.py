...
def train(Trainloader,dev_iter,config, model):
    '''
    :param x_data: 训练数据，size=样本数*时刻数*每个时刻的特征数
    ...
    for e in range(epoch):  # 每一次迭代都是针对全体数据做的,但是参数更新一batch一次
        for _, (trains, labels) in enumerate(Trainloader):  # 批数据的下标，该批数据的data及对应的label
            if use_gpu:
            ...
            if total_batch % 100 == 0:
                ...
                dev_acc, dev_loss = evaluate(config, model, dev_iter)  # 在验证集上评估
    ...
    plt_loss_and_acc(index, losslist=lossList, acclist=accList)
...
def evaluate(config, model, data_iter, test=False):
    '''
    data_iter：验证集iter或者测试集iter
    对验证集，返回：全体样本的准确率、平均batch loss
    对测试集，返回：全体样本的准确率、平均batch loss、sklearn的评价指标报告、混淆矩阵
    '''
    model.eval()
    loss_total = 0
